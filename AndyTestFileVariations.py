...
def find_theme(notes):
    ...
    # prints the matrix representation of the lookup table
    def print_grid(grid):
        print numpy.matrix(grid)
    
    # safely returns the data of the given index
    def get(r, c, grid):
    ...
    # stores the intermediate results of the caculations to avoid exponential runtime
    lookup_table = [[0 for j in xrange(len(notes))] for i in xrange(len(notes))]

    # calculate all possible repeating subsequences
